...
function sfd_add_language_files() {
	load_plugin_textdomain( 'secure-file-download' );
}
add_action('init', 'sfd_add_language_files');


/**
 * Output a MD5'd link to the file in place of the shortcode. 
 */
function sfd_shortcode_handler( $atts, $content = null )  {
	...
	return '<a href=' . site_url( 'download' ) .">$span$content</span></a>"; //?path=$path&type=$type&referer=$referer\">$span$content</span></a>";

}
add_shortcode( 'download', 'sfd_shortcode_handler' );


function sfd_generate_post_hash( $post_id, $post ) {
...
add_action( 'save_post', 'sfd_generate_post_hash', 10, 2 );

/** File Download Functions **/


function sfd_call_me_when_download_query_param_set() {
	...
	if ($type == "") $type = sfd_get_mime_type($extension);
...
/**
 * Takes an extension and returns the mime-type. 
 */
function sfd_get_mime_type( $extension ) {
